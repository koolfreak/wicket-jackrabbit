...
public class CustomDataStore implements DataStore
{
	...
	while (tokenizer.hasMoreTokens()) {
            ...
            path = new String(datastore); // make sure the directories are created
	}
    ...
    public DataRecord addRecord(InputStream input) throws DataStoreException {
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (!file.exists()) {
                    temporary.renameTo(file); // rename to move to final directory
                    if (!file.exists()) {
	...
	File file = directory; // search first to default dataStore
	...
	return file; // just in case we do not find the file
    }

    private File traverseDataStore(String string,File directories)
    {
	
	File file = directories; // new datastore to search
	boolean _exist = existFileInDatastore(file, string);
	if(_exist )
	{
	    file = new File(file, string.substring(0, 2));
